...
#define AMP_FILE_PATH		"/lib/firmware/amp.img"
#define AMP_PART		"amp"
...
int amp_cpus_on(void)
{
	...
	int totalsize;
	int ret = 0;
	void *file_header = NULL;
	loff_t len_read = 0, file_size = 0;
	char bootdev[8] = {0,};
	...
	/* set the block device and partition currently in use, according to devnum */
	sprintf(bootdev, "%s:%s", env_get("devnum") ? env_get("devnum") : "0", "6");
	ret = fs_set_blk_dev(env_get("devtype") ? env_get("devtype") : "mmc", bootdev, FS_TYPE_EXT);
	if (ret) {
		AMP_E("fs_set_blk_dev return error, err code: %d\n", ret);
		goto out2;
	}

	/* determine whether the amp file exists */
	ret = fs_exists(AMP_FILE_PATH);
	if (!ret) {
		AMP_E("file %s not exist!\n", AMP_FILE_PATH);
		goto out2;
	}

	/* simplify the code and avoid repeated judgments */
	fs_set_blk_dev(env_get("devtype") ? env_get("devtype") : "mmc", bootdev, FS_TYPE_EXT);

	/* get amp file size */
	ret = fs_size(AMP_FILE_PATH, &file_size);
	if (ret) {
		AMP_E("fs_size return error, err code: %d\n", ret);
		goto out2;
	}

	/* alloc memory */
	file_header = malloc(file_size);
	if (!file_header)
		goto out2;

	/* read amp image */
	fs_set_blk_dev(env_get("devtype") ? env_get("devtype") : "mmc", bootdev, FS_TYPE_EXT);
	ret = fs_read(AMP_FILE_PATH, (ulong)file_header, 0, file_size, &len_read);
	if (ret !=0 || len_read != file_size) {
		AMP_E("fs_read return error, err code: %d\n", ret);
		goto out2;
	}

	/* get totalsize */
	memcpy(hdr, file_header, FIT_HEADER_SIZE);
	...
	/* load image */
	fit = memalign(ARCH_DMA_MINALIGN, totalsize);
	if (!fit) {
		printf("No memory\n");
		ret = -ENOMEM;
		goto out2;
	...
	memcpy((fit + FIT_HEADER_SIZE), (file_header + FIT_HEADER_SIZE), (totalsize - FIT_HEADER_SIZE));
