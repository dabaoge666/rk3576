...
static int get_file_name(const char *path, char *file_name)
{
	char *tmp, *cursor;
	...
	cursor = tmp;
	memset(cursor, 0, strlen(path) + 1);
	strcpy(cursor, path);

	cursor = strtok(cursor, "/");
	while (cursor != NULL) {
		if (strstr(cursor, ".bmp"))
			break;
		cursor = strtok(NULL, "/");
	}

	strcpy(file_name, cursor);
	free(tmp);
	return 0;
}
...
static int load_bmp_logo_from_ext4_fs(struct logo_info *logo, const char *bmp_path)
{
	struct rockchip_logo_cache *logo_cache;
	struct bmp_header *header;
	void *dst = NULL, *pdst;
	...
	ret = get_file_name(bmp_path, bmp_name);
	if (ret)
		return ret;
	...
	if (ret) {
		printf("fs set blk dev failed\n");
		goto free_header;
	}

	ret = fs_read(bmp_path, (ulong)header, 0, RK_BLK_SIZE, &len_read);
	if (ret) {
		printf("fail to read %s header\n", bmp_path);
		goto free_header;
	}
	if (len_read != RK_BLK_SIZE) {
		printf("fail to read %s header\n", bmp_path);
		ret = -EINVAL;
		goto free_header;
	}
	...
	ret = fs_read(bmp_path, (ulong)pdst, 0, 0, &len_read);
	if (ret) {
		printf("fail to read %s header\n", bmp_path);
		goto free_header;
	}
...
int rockchip_show_logo(void)
{
	...
	list_for_each_entry(s, &rockchip_display_list, head) {
		...
		if (load_bmp_logo_from_ext4_fs(&s->logo, \
			env_get("uboot_logo_ext4fs_path") ? \
			env_get("uboot_logo_ext4fs_path") : "/boot/logo.bmp")) {
			printf("failed to load uboot logo from ext4fs\n");
		} else {
			ret = display_logo(s);
			if (ret == -EAGAIN)
				ms = s;
...
void rockchip_display_fixup(void *blob)
{
	...
	if (fdt_node_offset_by_compatible(blob, 0, "rockchip,drm-logo") >= 0) {
		list_for_each_entry(s, &rockchip_display_list, head) {
			ret = load_bmp_logo_from_ext4_fs(&s->logo, \
				env_get("kernel_logo_ext4fs_path") ? \
				env_get("kernel_logo_ext4fs_path") : "/boot/logo_kernel.bmp");
			if (ret < 0) {
