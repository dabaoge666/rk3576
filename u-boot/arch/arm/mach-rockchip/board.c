...
void lvds_gm8775c_init(struct udevice *dev)
{
	dm_i2c_reg_write(dev, 0x00, 0xAA);
	dm_i2c_reg_write(dev, 0x48, 0x02);
	dm_i2c_reg_write(dev, 0xB6, 0x20);
	dm_i2c_reg_write(dev, 0x01, 0x00);
	dm_i2c_reg_write(dev, 0x02, 0x00);
	dm_i2c_reg_write(dev, 0x03, 0x34);
	dm_i2c_reg_write(dev, 0x04, 0x88);
	dm_i2c_reg_write(dev, 0x05, 0x18);
	dm_i2c_reg_write(dev, 0x06, 0x1D);
	dm_i2c_reg_write(dev, 0x07, 0x00);
	dm_i2c_reg_write(dev, 0x08, 0x03);
	dm_i2c_reg_write(dev, 0x09, 0x06);
	dm_i2c_reg_write(dev, 0x0A, 0x1D);
	dm_i2c_reg_write(dev, 0x0B, 0x02);
	dm_i2c_reg_write(dev, 0x0C, 0x46);
	dm_i2c_reg_write(dev, 0x0D, 0x01);
	dm_i2c_reg_write(dev, 0x0E, 0x80);
	dm_i2c_reg_write(dev, 0x0F, 0x20);
	dm_i2c_reg_write(dev, 0x10, 0x20);
	dm_i2c_reg_write(dev, 0x11, 0x03);
	dm_i2c_reg_write(dev, 0x12, 0x1B);
	dm_i2c_reg_write(dev, 0x13, 0x03);
	dm_i2c_reg_write(dev, 0x14, 0x01);
	dm_i2c_reg_write(dev, 0x15, 0x23);
	dm_i2c_reg_write(dev, 0x16, 0x40);
	dm_i2c_reg_write(dev, 0x17, 0x00);
	dm_i2c_reg_write(dev, 0x18, 0x01);
	dm_i2c_reg_write(dev, 0x19, 0x23);
	dm_i2c_reg_write(dev, 0x1A, 0x40);
	dm_i2c_reg_write(dev, 0x1B, 0x00);
	dm_i2c_reg_write(dev, 0x1E, 0x46);
	dm_i2c_reg_write(dev, 0x51, 0x30);
	dm_i2c_reg_write(dev, 0x1F, 0x10);
	dm_i2c_reg_write(dev, 0x2d, 0x01);
}
...
__weak int rk_board_init(void)
{
	...
	struct udevice *bus;
	struct udevice *xl9555_dev, *xl8574_dev, *gm8775_dev;
	int ret = 0;
	uint8_t reg_value;
	...
	gpio_request(GPIO0_D2, "som_led1");
	gpio_direction_output(GPIO0_D2, 1);

	ret = uclass_get_device_by_seq(UCLASS_I2C, 0, &bus);
	if (ret) {
		pr_err("I2C bus request failed: %d\n", ret);
		return 0;
	}

	ret = i2c_get_chip(bus, 0x20, 1, &xl9555_dev);
	if (ret) {
		pr_err("I2C device request failed: %d\n", ret);
		return 0;
	}

	// Set XL9555 P02, P03(USB ETH power on) pins as output
	reg_value = dm_i2c_reg_read(xl9555_dev, 0x06);
	reg_value &= ~(unsigned int)(BIT(2) | BIT(3));
	dm_i2c_reg_write(xl9555_dev, 0x06, reg_value);

	// Set XL9555 P02, P03(USB ETH power on) pins as low level
	reg_value = dm_i2c_reg_read(xl9555_dev, 0x02);
	reg_value &= ~(unsigned int)(BIT(2) | BIT(3));
	dm_i2c_reg_write(xl9555_dev, 0x02, reg_value);

	mdelay(100);

	// Set XL9555 P02, P03(USB ETH power on) pins as high level
	reg_value = dm_i2c_reg_read(xl9555_dev, 0x02);
	reg_value |= (unsigned int)(BIT(2) | BIT(3));
	dm_i2c_reg_write(xl9555_dev, 0x02, reg_value);
	...
	// Set XL9555 P10, P11, P12, P13, P14 (Camera 0 ~ 4 PWDN) pins as output
	reg_value = dm_i2c_reg_read(xl9555_dev, 0x07);
	reg_value &= ~(unsigned int)(BIT(0) | BIT(1) | BIT(2) | BIT(3) | BIT(4));
	dm_i2c_reg_write(xl9555_dev, 0x07, reg_value);

	// Set XL9555 P10, P11, P12, P13, P14 (Camera 0 ~ 4 PWDN) pins as high level
	reg_value = dm_i2c_reg_read(xl9555_dev, 0x03);
	reg_value |= (unsigned int)(BIT(0) | BIT(1) | BIT(2) | BIT(3) | BIT(4));
	dm_i2c_reg_write(xl9555_dev, 0x03, reg_value);

	// Set XL9555 P04(FCS950U CHIP_EN) pins as output
	reg_value = dm_i2c_reg_read(xl9555_dev, 0x06);
	reg_value &= ~(unsigned int)BIT(4);
	dm_i2c_reg_write(xl9555_dev, 0x06, reg_value);

	// Set XL9555 P04(FCS950U CHIP_EN) pins as low level
	reg_value = dm_i2c_reg_read(xl9555_dev, 0x02);
	reg_value &= ~(unsigned int)BIT(4);
	dm_i2c_reg_write(xl9555_dev, 0x02, reg_value);
	...
	// Set XL9555 P04(FCS950U CHIP_EN) pins as high level
	reg_value = dm_i2c_reg_read(xl9555_dev, 0x02);
	reg_value |= (unsigned int)BIT(4);
	dm_i2c_reg_write(xl9555_dev, 0x02, reg_value);
	...
	ret = i2c_get_chip(bus, 0x21, 1, &xl8574_dev);
	if (ret) {
		pr_err("I2C device request failed: %d\n", ret);
		return 0;
	}

	// Set XL8574 P00 ~ P07 pins as low level
	reg_value = 0x00;
	dm_i2c_reg_write(xl8574_dev, 0x00, reg_value);

	mdelay(200);

	// Set XL9555 P15(LVDS_BL_EN) pins as output
	reg_value = dm_i2c_reg_read(xl9555_dev, 0x07);
	reg_value &= ~(unsigned int)BIT(5);
	dm_i2c_reg_write(xl9555_dev, 0x07, reg_value);

	// Set XL9555 P15(LVDS_BL_EN) pins as high level
	reg_value = dm_i2c_reg_read(xl9555_dev, 0x03);
	reg_value |= (unsigned int)BIT(5);
	dm_i2c_reg_write(xl9555_dev, 0x03, reg_value);

	// Set XL9555 P17(LVDS_EN) pins as output
	reg_value = dm_i2c_reg_read(xl9555_dev, 0x07);
	reg_value &= ~(unsigned int)BIT(7);
	dm_i2c_reg_write(xl9555_dev, 0x07, reg_value);

	// Set XL9555 P17(LVDS_EN) pins as low level
	reg_value = dm_i2c_reg_read(xl9555_dev, 0x03);
	reg_value &= ~(unsigned int)BIT(7);
	dm_i2c_reg_write(xl9555_dev, 0x03, reg_value);

	// Set XL8574 P05(SW_IN) , P07(FCS950U RESETN) and P06(RESX_EN) pins as high level
	reg_value = (unsigned int)BIT(5) | BIT(6) | BIT(7);
	dm_i2c_reg_write(xl8574_dev, 0x00, reg_value);

	ret = uclass_get_device_by_seq(UCLASS_I2C, 6, &bus);
	if (ret) {
		pr_err("I2C bus request failed: %d\n", ret);
		return 0;
	}

	ret = i2c_get_chip(bus, 0x2d, 1, &gm8775_dev);
	if (ret) {
		pr_err("I2C device request failed: %d\n", ret);
		return 0;
	} else {
		lvds_gm8775c_init(gm8775_dev);
	}
...
char *board_fdt_chosen_bootargs(void *fdt)
{
	...
	else if (!strcmp("nfs", env_get("devtype")))
		env_update("bootargs", "root=/dev/nfs");
